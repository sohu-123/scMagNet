...
def initialize_assignment_matrix(model, X_sc, X_st, n_neighbors=15):
    ...
    A_init = torch.tensor(
        np.random.normal(0, 1, (model.n_st_spots, model.n_sc_cells)),
        dtype=torch.float32)
...
# ── 变换函数：全部用 numpy，彻底避免 torch.quantile 大矩阵限制 ──────────

def transform_log1p_clipped(X: torch.Tensor, q: float = 0.995) -> torch.Tensor:
    """
    encoder 用的输入变换：
    先 per-gene clip 到 q 分位（numpy 实现，无大小限制），再 log1p。
    输出：连续浮点，数值稳定，供 encoder 使用。
    """
    X_np = X.numpy()
    # np.percentile 按列（基因），axis=0，keepdims 用于广播
    thresh = np.percentile(X_np, q * 100, axis=0, keepdims=True)  # (1, genes)
    X_clipped = np.minimum(X_np, thresh)
    return torch.FloatTensor(np.log1p(X_clipped))


def compute_loss_threshold(X_raw: torch.Tensor, q: float = 0.99) -> torch.Tensor:
    """
    Poisson loss 用的 robust 阈值：在原始 count（未 log）上计算。
    全程 numpy，无元素数量限制，返回标量 tensor。
    """
    val = float(np.percentile(X_raw.numpy(), q * 100))
...
class VAEImprovedSpatialModel(nn.Module):
    ...
    核心设计：双输入分离
    ┌─────────────────────────────────────────────────────────┐
    │  X_sc_enc  = log1p(clip(X_sc_raw))  → encoder 输入      │
    │  X_sc_raw  = 原始 count（normalize_total 后）→ loss 目标  │
    │                                                         │
    │  decoder 输出 f_vals（Softplus > 0）= Poisson λ          │
    │  Poisson NLL(target=X_sc_raw, λ=f_vals) ← 数学正确       │
    │  encoder 收到 log1p 值 ← 数值稳定，梯度正常              │
    └─────────────────────────────────────────────────────────┘
    """
    def __init__(self, n_sc_cells, n_st_spots, n_genes, embedding_dim=8,
        ...
        # decoder 输出 Poisson λ（Softplus 保证 > 0）
        # λ 的量纲 = 原始 count 量纲（normalize_total 后）
        ...
        # robust 阈值：在原始 count 分布上预计算，forward 里直接用
        self.register_buffer('loss_threshold', torch.tensor(float('inf')))

    def reparameterize(self, mu, logvar):
        return mu + torch.randn_like(mu) * torch.exp(0.5 * logvar)

    def encode_sc(self, X_sc_enc):
        """X_sc_enc: log1p 变换后的值，数值稳定"""
        h      = self.encoder(X_sc_enc)
        mu     = self.fc_mu(h)
    ...
    def poisson_nll_loss(self, y_true_raw, lam):
        """
        y_true_raw : 原始 count（非负整数或 normalize_total 后的浮点，未 log）
        lam        : decoder 输出的 Poisson λ（> 0）

        Poisson NLL = λ - y·log(λ)  [full=False 省略 lgamma(y+1)]
        数学假设：y ≥ 0，λ > 0。两者均满足，不需要 y 是严格整数。

        Robust 处理：
          y <= loss_threshold : 标准 Poisson NLL
          y >  loss_threshold : 归一化 L1，防止极端高表达基因梯度爆炸
        loss_threshold 在训练前由 compute_loss_threshold() 预计算写入，
        forward 里零开销，不触发任何大矩阵 quantile。
        """
        lam         = torch.clamp(lam, min=1e-8)
        mask_normal = (y_true_raw <= self.loss_threshold)

        loss_normal = F.poisson_nll_loss(
            torch.log(lam + 1e-8), y_true_raw,
            log_input=True, full=False, reduction='none'
        )
        # 离群区域：归一化 L1，梯度 ≈ 1/(y+1)，不随 y 爆炸
        loss_outlier = torch.abs(lam - y_true_raw) / (y_true_raw + 1.0)

        return torch.where(mask_normal, loss_normal, loss_outlier).mean()

    def loss_L1(self, X_sc_raw, f_vals):
        """loss_L1 用原始 count 作 target，f_vals 作 λ"""
        return self.poisson_nll_loss(X_sc_raw, f_vals)

    def loss_L1_cos(self, X_sc_enc, f_vals):
        """cosine loss 用 log1p 编码值，量纲一致"""
        return 1 - 0.5 * cosine_similarity(f_vals, X_sc_enc, dim=0).mean()

    def loss_L2(self, X_st_raw, f_vals, A_softmax=None):
        if self.lambda_f == 0:
        ...
        return self.poisson_nll_loss(X_st_raw, self.log_scale * X_st_pred)
    ...
    def loss_LM(self, X_sc, X_st):
        """LM 用的是 log1p 编码值做 cosine，量纲一致"""
        A_softmax = self.compute_softmax_A()
        if self.lambda_f == 0:
            G_pred = torch.matmul(A_softmax, X_sc)
        else:
            G_pred = torch.matmul(A_softmax, torch.sigmoid(self.f) * X_sc)
    ...
    def forward(self, X_sc_enc, X_sc_raw, X_st_enc, X_st_raw,
                z_pca=None, S=None, num_cell=None, f_vals=None, z_cell=None):
        """
        参数说明：
          X_sc_enc  : log1p 变换后的 sc 数据  → encoder 输入、cosine loss
          X_sc_raw  : 原始 count 的 sc 数据   → Poisson NLL target
          X_st_enc  : log1p 变换后的 ST 数据  → LM cosine loss
          X_st_raw  : 原始 count 的 ST 数据   → L2 Poisson NLL target
        """
        ...
        X_sc_enc = to_dt(X_sc_enc)
        X_sc_raw = to_dt(X_sc_raw)
        X_st_enc = to_dt(X_st_enc)
        X_st_raw = to_dt(X_st_raw)

        if self.predictA is not None:
            # 第二阶段：优化 A，LM 用 log1p 编码值做 cosine
            S        = to_dt(S)
            num_cell = to_dt(num_cell)
            z_st     = self.compute_st_embedding(z_cell)
            l3       = self.loss_L3(z_st, S)
            LM       = self.loss_LM(X_sc_enc, X_st_enc)
            LM1      = self.loss_LM(f_vals, X_st_enc)
            l_f      = self.custom_loss()
            row_sum_loss = self.row_sum_mse_loss(num_cell) if num_cell is not None \
                           else torch.tensor(0.0, device=X_sc_enc.device)
            ...
            # 第一阶段：训练 encoder/decoder
            # encoder 看 log1p 值（数值稳定）
            z_cell, mu, logvar = self.encode_sc(X_sc_enc)
            # decoder 输出 λ（Poisson rate，量纲 = raw count）
            f_vals = self.decode_expression(z_cell)

            # Poisson NLL：target = 原始 count，λ = decoder 输出
            l1 = self.loss_L1(X_sc_raw, f_vals)

            kl = self.kl_loss(mu, logvar)

            # cosine loss：用 log1p 编码值，与 f_vals 量纲对齐
            # f_vals 是 Poisson λ（raw count 量纲），对其做 log1p 再比较
            f_vals_enc = torch.log1p(f_vals)
            l4 = self.loss_L1_cos(X_sc_enc, f_vals_enc)

            if z_pca is None:
                raise ValueError("z_pca must be provided for alignment loss.")
            z_cell_proj = self.align_mlp(z_cell)
            z_pca_t     = torch.tensor(z_pca, device=X_sc_enc.device, dtype=X_sc_enc.dtype)
            loss_align  = F.mse_loss(z_cell_proj, z_pca_t, reduction='mean')
...
def train_vae_poisson_model(sc_path, st_path, C=None, num_cell=None,
    ...
    X_sc_raw, X_st_raw, Y, adata_sc, adata_ST, genes = load_and_align_data(
        sc_path, st_path, n_top_genes=n_top_genes,
        common_genes=common_genes, fix_genes=fix_genes)
    # X_sc_raw / X_st_raw : 原始 count（normalize_total 后），保持不动，供 Poisson loss 用

    # encoder 输入：log1p(clip)，数值稳定
    X_sc_enc = transform_log1p_clipped(X_sc_raw, q=0.995)
    X_st_enc = transform_log1p_clipped(X_st_raw, q=0.995)

    # Poisson loss robust 阈值：在原始 count 分布上计算，全 numpy，无大小限制
    sc_threshold = compute_loss_threshold(X_sc_raw, q=0.99)
    print(f"loss_threshold (99th pct of raw X_sc): {sc_threshold.item():.2f}")

    if predictA is not None:
        S = create_similarity_graph(Y.numpy()) if Y is not None \
            else torch.zeros(X_st_raw.shape[0], X_st_raw.shape[0])
        S = S.to(device)

    X_sc_enc = X_sc_enc.to(device)
    X_sc_raw = X_sc_raw.to(device)
    X_st_enc = X_st_enc.to(device)
    X_st_raw = X_st_raw.to(device)

    model = VAEImprovedSpatialModel(
        n_sc_cells=X_sc_raw.shape[0], n_st_spots=X_st_raw.shape[0],
        n_genes=X_sc_raw.shape[1], embedding_dim=embedding_dim,
    ...
    if z_pca is None:
        # PCA 初始化用 log1p 编码值（数值稳定）
        pca_sc = initialize_assignment_matrix(model, X_sc_enc, X_st_enc, n_neighbors=15)
    else:
        pca_sc = z_pca.copy()
    ...
    for epoch in range(epochs):
        ...
        if predictA is None:
            total, l1, l2, kl, loss_align = model(
                X_sc_enc, X_sc_raw, X_st_enc, X_st_raw,
                z_pca=pca_sc[:, :embedding_dim])
        else:
            total, LM, LM1, l_f, row_sum_loss, l3 = model(
                X_sc_enc, X_sc_raw, X_st_enc, X_st_raw,
                S=S, num_cell=num_cell, f_vals=f_vals, z_cell=z_cell)
...
def evaluate_predictions_numpy(
    ...
    if predictA is not None and cell_type_labels is not None:
        ...
        row_s = dec_pred_raw.sum(axis=1, keepdims=True)
        row_s = np.where(row_s == 0, 1.0, row_s)
        dec_pred_raw = dec_pred_raw / row_s
    ...
    if scembeding is not None and cell_type_labels is not None:
        ...
        if len(np.unique(y)) > 1:
            ...
            metrics['silhouette_index'] = float(
                silhouette_score(dist_mat, y, metric='precomputed'))
        else:
...
def generate_optimized_A_softmax_normalized(A0, adj_matrix, lambda_reg=10.0, lr=0.05, epochs=1000):
    ...
    S          = torch.nn.Parameter(
        torch.log(A0_tensor + 1e-8) + torch.randn_like(A0_tensor) * 0.01)
...
num_cell   = torch.tensor(Count_cell['n_cells'].values, device='cuda:5')

print("=" * 60)
print("Stage 1: training encoder / decoder")
print("=" * 60)
...
ckpt_path_1 = ('SpatialVG_improved_NMF/models/'
...
print("Stage 1 training completed and model saved.")

# ── 评估阶段：重新加载原始 count 和编码值 ──────────────────────
n_top_genes  = 5000
markers      = np.load('./20k_markers.npy')
common_genes = markers.tolist()
device       = torch.device('cuda:5' if torch.cuda.is_available() else 'cpu')

X_sc_raw, X_st_raw, Y, adata_sc, adata_ST, common_genes = load_and_align_data(
    sc_data_path, st_data_path, n_top_genes, common_genes=common_genes)
X_sc_enc = transform_log1p_clipped(X_sc_raw, q=0.995)
X_st_enc = transform_log1p_clipped(X_st_raw, q=0.995)

checkpoint   = torch.load(ckpt_path_1, map_location=device, weights_only=False)
...
# encoder 用 log1p 编码值
X_sc_enc_gpu = X_sc_enc.to(device).float()

with torch.no_grad():
    z_cell, _, _ = model.encode_sc(X_sc_enc_gpu)
    # f_vals 量纲 = 原始 count
...
A_hat_df = pd.DataFrame(A_hat_np, index=adata_st.obs_names, columns=adata_sc.obs_names)
A_hat_df = A_hat_df.loc[A_gt_deconv.index, :]
...
print("=" * 60)
print("Stage 2: optimising assignment matrix A")
print("=" * 60)
...
model, common_genes, losses = train_vae_poisson_model(
    ...
    f_vals=torch.tensor(predictsc,  device='cuda:5'),
    z_cell=torch.tensor(scembeding, device='cuda:5'),
    fix_genes=True
)

ckpt_path_2 = ('SpatialVG_improved_NMF/models/'
               'fix_enc_pca1_top5000_kl_soft_harm_getA2forzhou.pth')
torch.save({
    'model_state_dict': model.state_dict(),
    'common_genes':     common_genes,
    'training_losses':  losses,
}, ckpt_path_2)
print("Stage 2 training completed and model saved.")
...
A_hat_df  = A_hat_df / A_hat_df.sum(axis=0)
AA        = get_entropy_uniformity(A_hat_df.values)
index0    = np.where(AA < 0.87)[0]
A_hat_np1 = A_hat_df.iloc[:, index0]
...
row_s = A_dec_pred1.sum(axis=1, keepdims=True)
row_s = np.where(row_s == 0, 1.0, row_s)
A_dec_pred1 = A_dec_pred1 / row_s
